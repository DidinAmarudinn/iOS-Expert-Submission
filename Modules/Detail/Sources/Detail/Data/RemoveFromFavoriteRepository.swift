...
import Core
import Combine
public struct RemoveFromFavoriteRepository<DetailLocalDataSource: LocalDataSource, Transformer: Mapper >: Repository
where DetailLocalDataSource.Response == GameEntityCore, Transformer.Domain == GameDomainModel, Transformer.Entity == GameEntityCore {
    ...
    public func execute(request: GameDomainModel?) -> AnyPublisher<Bool, Error> {
        return _localeDataSource.delete(entity: _mapper.transformDomainToEntity(domain: request!)).eraseToAnyPublisher()
    }
}
